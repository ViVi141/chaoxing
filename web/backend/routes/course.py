...
async def get_course_list(
    ...
    try:
        ...
        if not current_user.config:
            raise HTTPException(status_code=400, detail="请先配置超星账号")
        ...
        if not config.cx_username or not config.cx_password_encrypted:
            raise HTTPException(
                status_code=400, detail="请先在配置管理中填写超星账号和密码"
            )
        ...
        if not cx_password:
            raise HTTPException(status_code=400, detail="密码解密失败，请重新配置密码")
        ...
        if not login_result["status"]:
            raise HTTPException(
                status_code=400, detail=f"登录超星失败: {login_result['msg']}"
            )
