...
async def get_tasks(
    ...
    page_size: int = Query(
        settings.PAGE_SIZE, ge=1, le=settings.MAX_PAGE_SIZE, description="每页数量"
    ),
...
async def create_task(
    ...
    max_tasks = await config_manager.get_config(
        db, "max_concurrent_tasks_per_user", settings.MAX_CONCURRENT_TASKS_PER_USER
    )

    result = await db.execute(
        select(Task).where(
            Task.user_id == current_user.id, Task.status.in_(["pending", "running"])
        )
    )
    ...
    # 创建任务
    task = Task(user_id=current_user.id, name=task_data.name, status="pending")
...
async def start_task(
    ...
    if not current_user.config or not current_user.config.cx_username:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="请先配置超星账号"
        )
...
async def pause_task(
    ...
    if task.status != "running":
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="只能暂停运行中的任务"
        )
...
async def cancel_task(
    ...
    if task.status in ["completed", "cancelled"]:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="任务已完成或已取消"
        )
