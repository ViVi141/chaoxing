        ...
        if task:
            ...
            if current_item:
                if item_current_time is not None and item_total_time is not None:
                    ...
                    # 基本进度
                    logger.info(
                        f"[Task {task_id}] {current_item}: {item_progress}% | 总进度: {progress}%"
                    )
...
class DetailedProgressCallback:
    ...
    def on_video_start(self, video_name: str, duration: int):
        """视频开始"""
        log_task_message(
            self.task_id, "INFO", f"🎬 开始观看视频: {video_name} (时长: {duration}秒)"
        )

    def on_video_progress(self, video_name: str, current: int, total: int):
        """视频进度更新"""
        video_progress = int((current / total) * 100) if total > 0 else 0
        overall_progress = self.base_progress + int(
            (video_progress / 100) * self.course_progress_range * 0.8
        )
    ...
    def on_document_start(self, doc_name: str, pages: int):
        """文档开始"""
        log_task_message(
            self.task_id, "INFO", f"📄 开始阅读文档: {doc_name} (共{pages}页)"
        )
    ...
    def on_document_progress(self, doc_name: str, current_page: int, total_pages: int):
        ...
        update_task_progress(
            self.task_id,
            self.base_progress
            + int((doc_progress / 100) * self.course_progress_range * 0.5),
            "running",
    ...
    def on_question_answer(
        self, question_idx: int, total_questions: int, question_type: str
    ):
        """答题进度"""
        task_progress = (
            int((question_idx / total_questions) * 100) if total_questions > 0 else 0
        )

        update_task_progress(
            self.task_id,
            self.base_progress
            + int((task_progress / 100) * self.course_progress_range * 0.3),
            "running",
    ...
    def on_course_progress(self, completed_points: int, total_points: int):
        ...
        overall_progress = self.base_progress + int(
            (course_progress / 100) * self.course_progress_range
        )
...
def execute_study_task(task_id: int, user_id: int):
    ...
    try:
        ...
        if not login_result["status"]:
            update_task_progress(
                task_id, 10, "failed", f"登录失败: {login_result['msg']}"
            )
        ...
        notopen_action = (
            config.notopen_action if hasattr(config, "notopen_action") else "continue"
        )
        ...
        for idx, course in enumerate(course_task):
            ...
            try:
                ...
                if current_task and current_task.status in ["paused", "cancelled"]:
                    log_task_message(
                        task_id, "WARNING", f"⚠️ 任务被{current_task.status}，停止执行"
                    )
                    logger.info(
                        f"任务{task_id}检测到状态为{current_task.status}，主动退出"
                    )
                    return  # 主动退出任务
            finally:
                db.close()
            ...
            log_task_message(
                task_id,
                "INFO",
                f"📚 开始学习课程 ({idx+1}/{len(course_task)}): {course['title']}",
            )
            ...
            try:
                ...
                success = process_course_with_detailed_progress(
                    chaoxing, course, speed, notopen_action, progress_callback
                )
                ...
                if success:
                    ...
                    failed += 1
                    log_task_message(
                        task_id, "WARNING", f"⚠️ 课程 {course['title']} 学习未完全成功"
                    )
        ...
        if completed == 0:
            ...
            update_task_progress(
                task_id, 100, "failed", f"所有课程失败: {error_detail}"
            )
        elif failed > 0:
            # 部分失败
            ...
            log_task_message(
                task_id,
                "WARNING",
                f"⚠️ 课程学习部分完成！成功 {completed}/{len(course_task)} 门课程，失败 {failed} 门",
            )
            update_task_progress(
                task_id,
                100,
                "completed",
                f"部分成功 ({completed}/{len(course_task)}): {error_detail}",
            )
        else:
            # 全部成功
            log_task_message(
                task_id,
                "INFO",
                f"🎉 所有课程学习完成！完成 {completed}/{len(course_task)} 门课程",
            )
        ...
        try:
            ...
            notification.send(
                f"超星学习任务完成！完成{completed}/{len(course_task)}门课程"
            )
        except Exception as e:
    ...
    try:
        ...
        def log_callback(level: str, message: str):
            ...
            if video_progress_match:
                ...
                update_task_progress(
                    ...
                    + int(
                        ...
                        * progress_callback.course_progress_range
                        * 0.8
                    ),
                    "running",
                    current_item=f"🎬 {video_name}",
            ...
            if "识别到视频任务" in message:
                ...
                log_task_message(
                    progress_callback.task_id, "INFO", "📝 识别到章节检测任务"
                )
