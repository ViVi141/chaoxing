...
from models import User, UserConfig, Task, TaskLog, SystemLog, EmailVerification, SystemConfig
...
class DatabaseMigrator:
    ...
    async def migrate_all_data(self):
        ...
        try:
            ...
            # 创建源数据库连接（同步）
            source_engine = create_engine(self.source_db_url.replace('aiosqlite', 'sqlite'))
            ...
            self._report_progress("迁移数据", 70, f"数据迁移完成，共 {total_migrated} 条记录")
...
async def test_migration():
    ...
    target_url = "postgresql+asyncpg://chaoxing_user:password@localhost:5432/chaoxing_db"
