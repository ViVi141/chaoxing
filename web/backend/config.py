...
class Settings(BaseSettings):
    ...
    DATABASE_URL: str = Field(
        default="sqlite+aiosqlite:///data/chaoxing.db", description="数据库URL"
    )
    ...
    CELERY_BROKER_URL: str = Field(
        default="filesystem://localhost/", description="Celery Broker"
    )
    CELERY_RESULT_BACKEND: str = Field(
        default="file://data/celery_results", description="Celery结果后端"
    )
    ...
    VERSION: str = Field(default="2.5.4", description="版本号")
    ...
    DEFAULT_ADMIN_PASSWORD: str = Field(
        default="Admin@123", description="默认管理员密码"
    )
    DEFAULT_ADMIN_EMAIL: str = Field(
        default="admin@example.com", description="默认管理员邮箱"
    )
    ...
    EMAIL_VERIFICATION_EXPIRE_MINUTES: int = Field(
        default=30, description="邮箱验证令牌过期时间（分钟）"
    )
    PASSWORD_RESET_EXPIRE_MINUTES: int = Field(
        default=30, description="密码重置令牌过期时间（分钟）"
    )

    # 任务配置
    MAX_CONCURRENT_TASKS_PER_USER: int = Field(
        default=3, description="每用户最大并发任务数"
    )
