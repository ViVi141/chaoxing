...
class CourseProcessor:
    ...
    def handle_not_open_chapter(
        ...
        if self.notopen_action == "retry":
            # 默认处理方式：重试
            if (
                not self.chaoxing.tiku
                or self.chaoxing.tiku.DISABLE
                or not self.chaoxing.tiku.SUBMIT
            ):
                self._log(
                    "ERROR",
    ...
    def process_job(
        self, course: Dict[str, Any], job: Dict[str, Any], job_info: Dict[str, Any]
    ) -> bool:
        """
        处理单个任务点
        ...
        try:
            ...
            while __point_index < total_points:
                ...
                self._update_progress(
                    f"处理章节 {__point_index + 1}/{total_points}", progress
                )
    ...
    def process_courses(self, courses: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        result = {"total": total, "completed": completed, "failed": failed}

        self._log(
            "INFO", f"课程处理完成: 总计{total}门, 成功{completed}门, 失败{failed}门"
        )
