...
def decode_course_list(html_text: str) -> List[Dict[str, str]]:
    ...
    for course in raw_courses:
        ...
        course_detail = {
            ...
            "desc": (
                course.select_one("p.margint10").attrs["title"]
                if course.select_one("p.margint10")
                else ""
            ),
...
def _extract_points_from_chapter(chapter_unit) -> List[Dict[str, Any]]:
    ...
    for raw_point in raw_points:
        ...
        if point.select_one("input.knowledgeJobCount"):
            ...
            point.select_one("span.bntHoverTips")
            and "解锁" in point.select_one("span.bntHoverTips").text
        ):
            need_unlock = True
        ...
        if (
            point.select_one("span.bntHoverTips")
            and "已完成" in point.select_one("span.bntHoverTips").text
        ):
            is_finished = True
...
def _process_read_task(card: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """处理阅读类型任务"""
    if not (
        card.get("type") == "read" and not card.get("property", {}).get("read", False)
    ):
        return None
