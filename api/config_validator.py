...
class ConfigValidator:
    ...
    def validate_notopen_action(action: str) -> Tuple[bool, str]:
        ...
        if action not in valid_actions:
            return False, f"无效的notopen_action: {action}，有效值: {', '.join(valid_actions)}"
    ...
    def validate_tiku_config(tiku_config: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        if provider not in valid_providers:
            return False, f"无效的题库provider: {provider}，有效值: {', '.join(valid_providers)}"
    ...
    @staticmethod
    def validate_notification_config(notification_config: Dict[str, Any]) -> Tuple[bool, str]:
        """
        验证通知配置
        ...
        if provider not in valid_providers:
            return False, f"无效的通知provider: {provider}，有效值: {', '.join(valid_providers)}"
